...
module pid_controller (
    ...
    // Output - 輸出
    output reg  signed [15:0] output     // PID output (Q8.8) - PID輸出
);
    ...
    wire signed [31:0] i_term_temp;     // Integral term temporary - 積分項臨時值
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (output_sum > $signed({1'b0, OUTPUT_MAX}))
                output <= OUTPUT_MAX;          // 限制在最大值
            else if (output_sum < $signed({1'b1, OUTPUT_MIN}))
                output <= OUTPUT_MIN;          // 限制在最小值
            else
                output <= output_sum[15:0];    // 正常輸出
        end
    end
